...
from call_cloud_function import call_cld_func
...
class Server(object):
    ...
    def _update_device_config(self, project_id, region, registry_id, device_id,
        ...
        print('The device ({}) has a temperature '
              'of: {}'.format(device_id, data['temp']))

        print('The device ({}) has a moisture of {}'.format(device_id,data['moisture']))

        if data['water_on']:
            print('The device ({}) has the water_pump on'.format(device_id))
        else:
            print('The device ({}) has the water_pump off'.format(device_id))

        if data['moisture'] > 200 and data['water_on']==True:
            # Turn off the water_pump.
            config_data = {'water_on': False}
            print('Setting swater_pump state for device', device_id, 'to off.')
        elif data['moisture'] < 150 and data['water_on']==False:
            # Turn on the water_pump
            config_data = {'water_on': True}
            call_cld_func({'deviceId':device_id,'moisture':data['moisture']})

            print('Setting water_pump state for device', device_id, 'to on.')
        else:
            # Moisture is OK, don't need to push a new config.
            if data['water_on']:
                print('has water pump running no config update required')
            else:
                print('Soil moisture is ok no need to update configuration')
            return
